...
if($do == 'installed') {
	...
	if (!empty($modules)) {
		foreach ($modules as $mid => $module) {
			...
			$modules[$mid]['official'] = empty($module['issystem']) && (strexists($module['author'], '-7') || strexists($module['author'], 'YuanCloud团队'));
...
if($do == 'permission') {
	...
	if (empty($module)) {
		message('你访问的模块不存在. 或许你愿意去YuanWX云服务平台看看. ', 'http://www.yuancloud.cn/?keyword=' . $_GPC['title']);
	}
...
<manifest xmlns="http://www.yuancloud.cn" versionCode="{$versions}">
...
class {$name}ModuleReceiver extends WeModuleReceiver {
	public function receive() {
		...
		//这里定义此模块进行消息订阅时的, 消息到达以后的具体处理过程, 请查看YuanWX文档来编写你的代码
	}
}
TPL;
	}
