...
if($do == 'display') {
	...
	if(!empty($list)) {
		foreach($list as &$li) {
			if(empty($li['email']) || (!empty($li['email']) && substr($li['email'], -6) == '123.cn' && strlen($li['email']) == 39)) {
...
if($do == 'post') {
	...
	if (checksubmit('submit')) {
		...
		if (!empty($_GPC)) {
			...
			if(!empty($_GPC['fanid'])) {
					...
					$_GPC['email'] = md5($_GPC['openid']) . '@123.cn';
				}
	...
	if(!empty($profile)) {
		if(empty($profile['email']) || (!empty($profile['email']) && substr($profile['email'], -6) == '123.cn' && strlen($profile['email']) == 39)) {
