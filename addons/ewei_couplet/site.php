...
class Ewei_coupletModuleSite extends WeModuleSite {
  ...
  private function check_weixin(){
    ...
    public function doMobileIndex() {
        ...
        if(!empty($fromfans)){
             //分享信息
            $sharetitle = "新年集对联，领大奖!";
            if($fans['status']>=1){
                 $sharedesc = "{$fromfans['nickname']}已经领取了集对联领{$reply['award_name']}，你还不赶紧来参加!";
                 $sharelink = $_W['siteroot'] . 'app/' . $this->createMobileUrl('index', array('id' => $id));
            }else{
                $sharedesc = "{$fromfans['nickname']}正在参与集对联领{$reply['award_name']}活动，快来帮{$fromfans['nickname']}抽取下联吧！";
           ...
           $helped = !empty($help);
             ...
             if($fans['status']>=1){
             //分享信息
                 $sharedesc = "哈哈, 我已经领取了集对联领{$reply['award_name']}，你还不赶紧来参加!";
                 $sharelink = $_W['siteroot'] . 'app/' . $this->createMobileUrl('index', array('id' => $id));
             }
             else{
                 $sharedesc = "我正在参与集对联领{$reply['award_name']}活动，快来帮我抽取下联吧！";
