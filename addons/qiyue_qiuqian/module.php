...
class Qiyue_qiuqianModule extends WeModule {
    public function settingsDisplay($settings) {
        ...
        if (checksubmit()) {
            ...
            $links = trim($_GPC['links']);
            $check = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('qiyue_qiuqian') . " WHERE uniacid=:uniacid", array(':uniacid' => $_W['uniacid']));
            if ($check) {
                pdo_update('qiyue_qiuqian', array('morepic' => $morepic, 'links' => $links, 'qiantype' => intval($_GPC['qiantype'])), array('uniacid' => $_W['uniacid']));
            }
            else {
                $add['uniacid'] = $_W['uniacid'];
                $add['morepic'] = $morepic;
                $add['links'] = $links;
                $add['qiantype'] = intval($_GPC['qiantype']);
        ...
        if (empty($settings)) {
            $settings = array(
                ...
                'imgUrl' => tomedia(MODULE_URL . 'icon.jpg'),
            );
        }
