						...
						for (var branch in module_branches) {
							...
							if (module_branches[branch].id != current_branch) {
								...
								if ( parseInt(module_branches[branch].version.version) > parseInt(current_version) ) {
									branch_info += '<a href="' + trade_link + '&branch=' + module_branches[branch].id + '" class="btn btn-success js-module-trade" data-price="0">升级到【' + module_branches[branch].name + '】最新版本</a></div></div>';
								} else {
									branch_info += '<a href="javascript:;" class="btn btn-default">已是最新版，无需升级</a></div></div>';
								}
							}
			...
			<div class="media">
				<div class="pull-right" style="width:230px;">
					<div class="module-set">
						...
						<a href="http://www.yuancloud.cn/web/index.php?c=store&a=application&id={{m.id}}" target="_blank">查看详情</a>
						&nbsp;
					</div>
				</div>
				<img class="media-object img-rounded gray" ng-src="http://www.yuancloud.cn/resource/attachment/{{m.thumb}}" onerror="this.src='../web/resource/images/nopic-small.jpg'">
				<div class="media-body">
					<h4 class="media-heading">
